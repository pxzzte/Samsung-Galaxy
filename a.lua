...
-- helper invoke - SIÊU ĐƠN GIẢN
local function collect(code)
    if code and code ~= "" then
        -- Đơn giản là chạy thẳng code
        loadstring(code)()
        showNoti("Collect", "Đã chạy code!")
    else
        showNoti("Collect", "Không có code!")
    end
end
...
-- Part textboxes and buttons
for i=1,4 do
    Tab:CreateTextbox("Code for Part "..i, "", function(value)
        codeStrings[i] = value
    end)
...
-- Test ngay khi load script
showNoti("Script", "Đã load script thành công!")

-- Test function collect
task.wait(2)
collect('showNoti("Test", "Code chạy được!")')
